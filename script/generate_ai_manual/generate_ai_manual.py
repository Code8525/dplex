#!/usr/bin/env python3
"""
Скрипт для генерации инструкции для ИИ ассистентов на основе структуры проекта Dataplex
...
class AIManualGenerator:
    ...
    def get_module_purpose(self, module_path: str) -> str:
        ...
        module_purposes = {
            ...
            'cli': 'Интерфейс командной строки. CLI команды для работы с Dataplex.',
    ...
    def generate_manual_header(self) -> str:
        ...
        return f'''# 🤖 Dataplex AI Assistant Manual

**Автоматически сгенерированный мануал для работы с проектом Dataplex**

📅 **Дата генерации:** {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
🏗️ **Проект:** Dataplex - Enterprise-grade data layer framework for Python
...
**Dataplex** - это enterprise-фреймворк для создания слоя данных в Python приложениях. 
...
'''
    ...
    def save_manual(self, output_file: str = "DATAPLEX_AI_MANUAL.md") -> None:
...
def main():
    ...
    try:
        output_file = input("📄 Имя выходного файла (по умолчанию 'DATAPLEX_AI_MANUAL.md'): ").strip()
        if not output_file:
            output_file = "DATAPLEX_AI_MANUAL.md"
